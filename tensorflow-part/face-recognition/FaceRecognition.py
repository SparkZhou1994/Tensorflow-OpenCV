# 8-5
# 1 data yale 2 train label 3 cnn
...
train_data = np.random.permutation(train_data) # 将数据无序排列
...
test_labels_input = np.zeros((165,15))
for i in range(0.64):
...
# run acc
init = tf.group(tf.global_variables_initializer(),tf.local.variables_initializer())
